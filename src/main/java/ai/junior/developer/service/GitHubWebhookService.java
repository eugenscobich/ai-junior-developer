    ...
    public void handleWebhook(String payload) throws Exception {
        ...
        if (commentNode != null && prNode != null) {
            ...
            if (prUserId.equals(applicationPropertiesConfig.getGithub().getUserId())
                ...
                var line = commentNode.get("line").toString();
                var path = commentNode.get("path").toString();
                log.info("Message: {} is added by another user to pr create the by ai-junior-developer", message);
                var prDescription = prNode.get("body").toString();
                ...
                if (matcher.find()) {
                    ...
                    if (threadId != null) {
                        ...
                        var finalPath = "Line: " + line + "\n\r" + "Path: " + path + "\n\r\n\r" + message;

                        var result = assistantService.executePrompt(finalPath, assistant.id(), threadId);
                        var prUrl = commentNode.get("pull_request_url").textValue();
                        gitHubService.addComment(prUrl, commentId, result);
                    }
                }
